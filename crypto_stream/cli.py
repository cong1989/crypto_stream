...
def run_streamer():
    ...
    from crypto_stream.market_data.streaming.kafka_streamer import main as streamer_main
    streamer_main()

def run_consumer():
    """Run the sampling recorder consumer"""
    from crypto_stream.market_data.processing.sampling_recorder_consumer import main as consumer_main
    consumer_main()
...
def start():
    ...
    try:
        ...
        tardis_check = subprocess.run(['docker', 'ps', '-q', '--filter', 'ancestor=tardisdev/tardis-machine'], 
        ...
        if not tardis_check.stdout:
            ...
            tardis_cmd = [
                ...
                '-e', f"TM_API_KEY={os.environ.get('TM_API_KEY')}",
...
def stop():
    ...
    try:
        ...
        # Check Tardis
        tardis = subprocess.run(['docker', 'ps', '-q', '--filter', 'ancestor=tardisdev/tardis-machine'], 
                              capture_output=True, text=True)
        if tardis.stdout:
...
def force_stop():
    ...
    # Force stop Tardis
    subprocess.run(['docker', 'kill', '$(docker ps -q --filter ancestor=tardisdev/tardis-machine)'], 
                  shell=True)
...
def status():
    ...
    try:
        kafka_process = subprocess.run(['pgrep', '-f', 'kafka.Kafka'], capture_output=True)
        click.echo(f"Kafka: {'Running' if kafka_process.returncode == 0 else 'Stopped'}")
    except:
    ...
    try:
        zk_process = subprocess.run(['pgrep', '-f', 'zookeeper'], capture_output=True)
        click.echo(f"Zookeeper: {'Running' if zk_process.returncode == 0 else 'Stopped'}")
    except:
