...
class SamplingRecorderConsumer:
    def __init__(self, data_dir):
        self._data_dir = Path(data_dir)
        self._consumer = create_kafka_consumer()
        self._kafka_topics = ['crypto-ticks-binance-futures-quote']
        self._consumer.subscribe(self._kafka_topics)
        self._cache = EnhancedRedisTickCache()
        self._writer = DiskWriter(self._data_dir)
    ...
    async def run(self):
        ...
        try:
            ...
            while True:
                try:
                    msg = await to_thread(self._consumer.poll, 1.0)
                    if msg is None:
                        continue
...
def main():
    ...
    log_dir = os.path.join(get_recording_options()['recorder_consumer_dir'],'logs')
    ...
    print('start main')
    consumer = SamplingRecorderConsumer(data_dir = get_recording_options()['recorder_consumer_dir'])
    try:
