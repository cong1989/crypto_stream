...
class RedisTickCache:
    ...
    def get_and_clear_ticks(self, cache_key):
        ...
        try:
            ...
            safety_margin = get_redis_options()['redis_tick_cache'].get('safety_margin', 1000)
            ...
            if clear_up_to > last_processed:
                ...
                pipe.set(last_idx_key, clear_up_to - 1)  # -1 because clear_up_to is kept
                pipe.expire(last_idx_key, get_redis_options()['redis_tick_cache']['redis_expiry'])
